...
source for product rules, tests, reviews, and releases. Keep each rule in its owning document and
link to it elsewhere.

Use ordinary technical language. Project-specific terms are useful when they name an existing
type, field, module, or enduring rule; define them at first use. Prefer concrete inputs, checks,
and effects to metaphors, new taxonomies, and unexplained shorthand.
